...
describe('RoomService', () => {
  ...
  it('throw BadRequestException for invalid password in private room', () => {
  ...
  it('allow a player to leave a room', () => {
    const room = roomService.createRoom();
    roomService.joinRoom(room.id, 'player1');
    roomService.leaveRoom(room.id, 'player1');
    const updatedRoom = roomService.findRoom(room.id);
